...
class Token {
  ...
  bool get _hasTokenReferences => value is String && (value as String).hasTokenReferences;
  ...
  bool get _hasMathExpression => value is String && (value as String).isMathExpression;
  ...
  Token resolveAllReferences(Map<String, Token> tokenMap) {
    ...
    if (token._hasTokenReferences == true) {
      if (token.valueAsString?.isTokenReference == true) {
        final reference = tokenMap[token.valueByRef]?.resolveAllReferences(tokenMap);
        if (reference == null) {
  ...
  Token _resolveMathExpression(Map<String, Token> tokenMap) {
    ...
    final isRound = valueAsString!.startsWith('round(');

    // Split on expression and parse the left and right side
    // We search for: a space, then a valid operator (*/+-), then a space
    final splitted = () {
      if (!isRound) return valueAsString!;
      return valueAsString!.substring(6, valueAsString!.length - 1);
    }()
        .split(RegExp(r'\s[*/+-]\s'));
    ...
    if (isRound) solved = solved.roundToDouble();
...
String? _resolveColorValue(String initialValue, Map<String, Token> tokenMap) {
  ...
  while (match != null) {
    ...
    final color =
        ColorValue.maybeParse(reference.value)?.applyStudioExtension(reference.extensions);
    if (color == null) {
      return null;
      // throw ResolveTokenException(
    ...
    final isRgb = value.substring(max(0, match.start - 7), match.start).contains('rgba(');
...
String _getVariableName(String path, String name) {
  final parts = path.split('.').where((e) => e.isNotEmpty).map((e) => e.alphanumeric).toList()
